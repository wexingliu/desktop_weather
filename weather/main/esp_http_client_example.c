...
static void https_with_url(void)
{
    ...
    if (dstBuf == NULL) {
        ESP_LOGE(TAG, "malloc for dstBuf failed");
        return;
    }
    memset(dstBuf, 0, 1024);

    /* 增加调试输出级别，便于定位 esp-tls / esp_http_client 的内部日志 */
    esp_log_level_set("esp_http_client", ESP_LOG_DEBUG);
    esp_log_level_set("esp_tls", ESP_LOG_DEBUG);

    /* 明确指定 host，并尝试使用全局 CA 存储 + crt bundle */
    esp_http_client_config_t config = {
        .url = "https://devapi.qweather.com/v7/weather/now?location=101100106&key=18885095bade4711969ee90056008044",
        ...
        if (Z_STREAM_END == ret) { /* 解压成功 */
            ...
            if (root) {
                ...
                if (now) {
                    ...
                    if (tmp && tmp->valuestring) ESP_LOGI(TAG, "温度：%s", tmp->valuestring);
                    if (hum && hum->valuestring) ESP_LOGI(TAG, "湿度：%s", hum->valuestring);
                }
                cJSON_Delete(root);
            } else {
...
static void obtain_time(void)
{
    ...
    while (retry < retry_count) {
        ...
        if (timeinfo.tm_year > (2016 - 1900)) { // 年份检查，确保已同步
    ...
    if (retry == retry_count) {
        ESP_LOGW(TAG, "Failed to synchronize time, TLS certificate validation may fail");
    }
}
